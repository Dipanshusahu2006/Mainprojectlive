...
function Header({ setSearchQuery }){
  ...
  const toggleMenu = () => setMenuOpen(prev => !prev);
      ...
      const Imges = {
      img123 : "https://your-daily-need.netlify.app/static/media/icon-headphone%20-%20Copy.c3fb047a0eb9f39bc24262dda11da296.svg"
     }


    async function Profiles() {
  const profilename = await fetch(`https://main-projectnode.vercel.app/user/Get/${Id}`);
  ...
  setslicenurl(SliceNames?.Data || {});
      ...
      async function Cart () {
        ...
        const Cartitmes = await Cartnumber.json();
        setCartnumbers(Cartitmes.Data || [])
      }
       async function Bells () {
        const Bellnumber = await fetch("https://main-projectnode.vercel.app/adminreply/Get")
        const Bellitmes = await Bellnumber.json();
        setBelll(Bellitmes.Data || [])
      }
  ...
  return(
   ...
   <div className="logos">
    <ul>
<li><NavLink to={"productcart"}><span>{Cartnumbers.length}</span><i className="fa-solid fa-cart-shopping"></i></NavLink></li>
<li><NavLink to={"/whislist"} ><span>{totalUniqueItems}</span><i className="fa-solid fa-heart"></i></NavLink></li>
<li><NavLink to={"/Myenqries"} ><span>{Bell.length}</span><i className="fa-solid fa-bell"></i></NavLink></li>
<li className="Hiddenprofilebuttone1" onClick={Slice3} style={buttne3}><i className="fa-solid fa-user"></i></li>
    <li className="Hiddenprofilebuttone2" onClick={Slice4} style={buttne4}><i className="fa-solid fa-user"></i></li>
 
</ul>
 {  !Id ? <>
       <button className="Login-buttone" style={LoginButtone}><Link to={"/Login"}>Log-in</Link></button>
     </> : <>
     <button className="btn1" onClick={Slice1} style={buttne1}>{Slicename}</button>
    <button className="btn2" onClick={Slice2} style={buttne2} >{Slicename}</button>
    </>
    }
   </div>
   </div>
   <div className="underhedaer" style={
          isMobile
   ...
   <div className="main-menu">
                ...
                <div className="Category">
                    ...
                    <button className="close-menu" onClick={() => setMenuOpen(false)}>&times;</button>
                </div>
                </div>
    <div className="menu">
<ul>
...
<div className="main-micro">
                <div className="mICROPHONE">
                    <img src={Imges.img123} alt="MICROPHONE"/>
                    </div>
   ...
   <div className="PROFILES" style={profiles} >
    <div className="profilesimeges">
      <img src={slicenurl.profileimage}></img>
