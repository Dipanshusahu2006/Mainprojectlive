...
function Productcart() {
 ...
 const [Profilecheckout, setProfilecheckout] = useState({});
  ...
  async function ProfileChekout() {
  const profileUser = await fetch(`https://main-projectnode.vercel.app/user/Get/${Id}`);
  ...
  const DeletesCart = async (id) => {
    try {
      const res = await fetch(`https://main-projectnode.vercel.app/cart/Delete/${id}`, {
        method: "DELETE",
      });
  ...
  />
  <meta name="keywords" content="shopping cart, checkout, online cart, myshop" />
  ...
  />
      ...
      <div className="cart-container">
        <div className="cart-items-section">
          ...
          {Cartproduct.map((item) => (
            <div className="cart-item" key={item._id}>
              ...
              <button onClick={() => DeletesCart(item._id)} className="remove-button">
                Remove
              </button>
            </div>
          ))}
        ...
        <div className="price-details-section">
          ...
          <button className="place-order-btn" onClick={Chekout}>Proceed to Checkout</button>
        </div>
      </div>
    </>
  );
